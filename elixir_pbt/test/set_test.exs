defmodule ElixirPbt.SetTest do
  ...
  property "A Set that has an term added to it should include that term as a member" do
    check all(
            list <-
              term()
              |> uniq_list_of()
  ...
  # This feels more property like, more substantial claim
  property "A Set should not change when a term put that is already present in the set" do
    check all(
            terms <-
              term()
              |> uniq_list_of()
